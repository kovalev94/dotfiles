#+TITLE: =/home/kovalev/.dotfiles=
My Guix SD and Guix Home configuration and relative modules.

* INFO
At this moment I don't know how to manipulate sensitive data - <br>
So \*.nmconnection files should be added, after system reconfiguring/building, manually.
Rclone.conf, syncthing certificates and keys, should be added before home reconfiguring, manually.

Also, for now - doom emacs should copied or installed manually,
after system or home reconfiguring

* INSTALLATION
To recreate/deploy system from install iso, you should do the following steps:
1. Clone this repository
   #+BEGIN_SRC
git clone https://github.com/kovalev94/.dotfiles.git
   #+END_SRC
2. Set enviroment variable - DOTFILES_DIR with path to this repository
   #+BEGIN_SRC
export DOTFILES_DIR=/path/to/this/repo
   #+END_SRC
3. Manually create disk partitions, LVM volumes, etc., and create filesystems.
4. Start cow-store on mnt
   #+BEGIN_SRC
herd start cow-store /mnt
   #+END_SRC
5. Initialize the desired system with /path/to/this/repo directory prepended to %load-path
   #+BEGIN_SRC
guix system -L /path/to/this/repo/ init \
    /path/to/this/repo/guix-config/systems/desired-system.scm /mnt
   #+END_SRC
6. After install reboot into new system
7. Login as root and setup password for root and if needed for other users
8. If system definition doesn't have guix-home-service Setup home enviroment:
   1. Clone this repository
      #+BEGIN_SRC
git clone https://github.com/kovalev94/.dotfiles.git
      #+END_SRC
   2. Set enviroment variable - DOTFILES_DIR with path to this repository
      #+BEGIN_SRC
export DOTFILES_DIR=/path/to/this/repo
      #+END_SRC
   3. Setup desired home environment with /path/to/this/repo directory prepended to %load-path
      #+BEGIN_SRC sh
guix home -L /path/to/this/repo/guix-config reconfigure \
    /path/to/this/repo/guix-config/kovalev/homes/desired-home.scm
      #+END_SRC
